...
/**
 * Plugin config schema. `dsh-width` carries no host-side tunables of its own —
 * the two width values live in the durable settings namespace (see
 * `WidthSettingsSchema`) — so this schema is intentionally empty and exists
 * only to satisfy the Cordis function-plugin contract (`Config` schema +
 * `apply(ctx, config)`). Any future host-side option belongs here.
 */
 ...
 * @param _config - Validated plugin config (unused; see {@link Config}).
 */

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * column.
     *
     * The composer width is pinned on the composer stack
     * (`--dsh-composer-card-max-width`) while the card itself stays at 100% of
     * the stack — setting the variable alone to `input%` would shrink the card
     * twice (stack then card ≈ 64% effective, nearly unchanged from the default
     * 780px). On the hero (new-session) page the workspace / agent-preset row
     * lives inside the same stack, so it already shares the stack width; the
     * composer card sits inside the InputBar root which carries a
     * `--dsh-composer-side-clearance` horizontal padding, so the menu row gets
     * the same side margin to line up flush with the input box.
     */
    function applyWidths(content, input) {
      var css = [
        '[data-conversation-scroll]{' +
          '--dsh-chat-content-width:' + content + '%!important;' +
          '--dsh-composer-card-max-width:' + input + '%!important;' +
          '}',
        '[data-chat-flow]{max-width:' + content + '%!important}',
        '[data-composer-card]{max-width:100%!important}',

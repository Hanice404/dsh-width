...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Apply the two widths by overriding the width CSS variables on the
     * stable `[data-conversation-scroll]` element (the conversation scroll
     * body is the nearest common ancestor of both the content column and the
     * composer card). Both values are expressed as a percentage of the
     * center column.
     */
    function applyWidths(content, input) {
      var css = '[data-conversation-scroll]{' +
        '--dsh-chat-content-width:' + content + '%;' +
        '--dsh-composer-card-max-width:' + input + '%;' +
        '}';

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Apply the two widths. Both values are a percentage.
     *
     * The composer stack keeps its stock width in every state — nothing else
     * on the page (hero welcome shell, in-session layout) is touched. Only the
     * input card itself is scaled via `max-width: <input>%`, and the message
     * column via `--dsh-chat-content-width` / `[data-chat-flow]`.
     *
    ...
    function alignHeroMenuRow() {
      ...
      var delta = Math.round(card.getBoundingClientRect().left - row.getBoundingClientRect().left);
      if (row.style.marginLeft !== delta + 'px') row.style.marginLeft = delta + 'px';
    }
    ...
    /**
     * Keep the hero menu row aligned across hero mount/unmount, settings
     * changes, and window resizes. Returns the disposer.
     */

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * column.
     *
     * Why the direct max-width overrides: `--dsh-composer-card-max-width` is
     * consumed at TWO levels — the composer stack and the card inside it — so
     * setting the variable alone to `input%` shrinks the card twice (e.g. 80%
     * on the stack then 80% of that on the card ≈ 64% effective, nearly
     * indistinguishable from the default 780px). We therefore pin the stack to
     * full width (variable = 100%) and set the card's own `max-width` to the
     * real percentage; the same belt-and-braces `!important` override is
     * applied to the message column (`[data-chat-flow]`).
     */

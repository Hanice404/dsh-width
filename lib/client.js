...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Apply the two widths. Both values are a percentage of the center
     * column.
     *
     * Active (in-session) layout keeps the composer stack full width and sets
     * the card's own `max-width` to `input%` — the card stays centered inside
     * the full-width stack. (Pinning the stack itself to `input%` instead made
     * the in-session input box left-align and "out of control".)
     *
     * Hero (new-session) layout: the stack is the `.composerHero` wrapper, so
     * we narrow the stack itself to `input%` (centered by the stock
     * `align-self:center`) so the workspace / agent-preset row — which lives
     * inside the same stack — lines up with the input card; the card then sits
     * inside the InputBar root which carries a
     * `--dsh-composer-side-clearance` horizontal padding, so the menu row gets
     * the same side margin to flush with the input box.
     */
    function applyWidths(content, input) {
      var css = [
        ...
        '[data-phase="hero"] [data-composer-seat] > div{' +
          'width:' + input + '%!important;' +
          '}',
        '[data-phase="hero"] [data-composer-seat] > div > div:has(> button[aria-haspopup="menu"]){' +
          'margin:4px var(--dsh-composer-side-clearance) 0!important;' +
          '}',
      ].join('\n');
